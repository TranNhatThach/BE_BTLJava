...
public class TuyenDungService {
    ...
    public TuyenDungResponse<String> guiLoiMoiTrucTiep(TuyenDungTrucTiepRequest request) {
        ...
        UngTuyen ungTuyen = UngTuyen.builder()
                ...
                .mucHocPhiDeXuat((request.getNganSachMin() != null && request.getNganSachMax() != null) ? (request.getNganSachMin() + request.getNganSachMax()) / 2 : request.getNganSachMin())
        ...
        notificationService.guiThongBao(giaSu.getTaiKhoan(), 
                "Học viên " + hocVien.getTaiKhoan().getHoTen() + " vừa gửi cho bạn một lời mời dạy trực tiếp môn " + monHoc.getTenMon(), 
                "TUYEN_DUNG");
    ...
    public LopHoc giaSuDongY(Integer maGiaSu, Integer maYeuCau) {
        ...
        notificationService.guiThongBao(yeuCau.getHocVien().getTaiKhoan(), 
                "Gia sư " + ungTuyen.getGiaSu().getTaiKhoan().getHoTen() + " đã đồng ý dạy lớp " + yeuCau.getMonHoc().getTenMon(), 
                "LOP_HOC");
    ...
    public void giaSuTuChoi(Integer maGiaSu, Integer maYeuCau) {
        ...
        notificationService.guiThongBao(yeuCau.getHocVien().getTaiKhoan(), 
                "Gia sư " + ungTuyen.getGiaSu().getTaiKhoan().getHoTen() + " đã từ chối lời mời dạy môn " + yeuCau.getMonHoc().getTenMon(), 
                "TUYEN_DUNG");
    }
    ...
    public TuyenDungResponse<String> giaSuUngTuyen(com.btljava.GiaSu.dto.GiaSuUngTuyenRequest request) {
        ...
        // Thông báo cho học viên
        notificationService.guiThongBao(yeuCau.getHocVien().getTaiKhoan(), 
                "Gia sư " + giaSu.getTaiKhoan().getHoTen() + " vừa ứng tuyển vào yêu cầu tìm gia sư môn " + yeuCau.getMonHoc().getTenMon(), 
                "TUYEN_DUNG");
    ...
    public LopHoc hocVienDuyet(Integer maHocVien, Integer maGiaSu, Integer maYeuCau) {
        ...
        notificationService.guiThongBao(ungTuyen.getGiaSu().getTaiKhoan(), 
                "Học viên " + yeuCau.getHocVien().getTaiKhoan().getHoTen() + " đã phê duyệt đơn ứng tuyển của bạn cho môn " + yeuCau.getMonHoc().getTenMon(), 
                "LOP_HOC");
    ...
    public void hocVienTuChoi(Integer maHocVien, Integer maGiaSu, Integer maYeuCau) {
        ...
        // Thông báo cho gia sư
        notificationService.guiThongBao(ungTuyen.getGiaSu().getTaiKhoan(), 
                "Học viên " + yeuCau.getHocVien().getTaiKhoan().getHoTen() + " đã từ chối đơn ứng tuyển của bạn cho môn " + yeuCau.getMonHoc().getTenMon(), 
                "TUYEN_DUNG");
    }
}
